...
export const to64 = (b) =>
  btoa(Array.from(new Uint8Array(b), (c) => String.fromCharCode(c)).join(""));
...
export async function deriveKey(passphrase, salt) {
  ...
  return crypto.subtle.deriveKey(
    { name: "PBKDF2", salt: from64(salt), iterations: 600000, hash: "SHA-256" },
    material,
...
export async function seal(value, key) {
  ...
  const ciphertext = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv, additionalData: enc.encode("family-vault:v1") },
    key,
      ...
      await crypto.subtle.decrypt(
        {
          name: "AES-GCM",
          iv: from64(box.iv),
          additionalData: enc.encode("family-vault:v1"),
        },
        key,
        from64(box.ciphertext),
      ),
    ),
  );
}
...
export function events(records) {
  return records
    .filter((r) => r.status === "active" && !r.isExample)
    .flatMap((r) =>
      [
        ["due", r.dueDate],
        ...
        .filter(
          ([kind, date]) =>
            date && !(kind === "due" && r.paymentStatus === "paid"),
        )
        .map(([kind, date]) => ({ id: r.id, name: r.name, kind, date })),
    )
      ...
      await crypto.subtle.encrypt(
        {
          name: "AES-GCM",
          iv,
          additionalData: enc.encode("family-vault:document:v1"),
        },
        key,
        bytes,
      ),
...
export async function unsealBytes(box, key) {
  if (box.v !== 1) throw Error("Unsupported document format");
...
export function normalize(value) {
  if (
    ...
    value.records.length > 2000 ||
    value.payments.length > 10000
  )
    throw Error("Invalid vault data");
  value.documents = Array.isArray(value.documents) ? value.documents : [];
  value.groups = Array.isArray(value.groups)
  ...
  for (const payment of value.payments) {
    if (
      ...
      typeof payment.recordId !== "string" ||
...
export function allEvents(value) {
  return [
